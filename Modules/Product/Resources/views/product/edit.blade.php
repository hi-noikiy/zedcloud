...
@section('content')
    <div class="table-responsive">
        <form class="form-horizontal" method="post" action="{{ route('product.product.update', ['_product_id' => $product->_id]) }}" enctype="multipart/form-data">
                ...
                <div class="col-sm-10">
                    <select class="form-control" name="sale_type">
                        ...
                        @foreach($sale_types as $key=>$sale_type)
                            <option value="{{ $key }}" @if($key == $product->sale_type) selected @endif>{{ $sale_type }}</option>
                        @endforeach
                    </select>
            ...
            <div class="form-group {{ $errors->has('cover') ? 'has-error' : '' }}">
                ...
                <div class="col-sm-10">
                    <input type="file" class="form-control" name="cover" value="{{ $product->cover }}">
                    @if ($errors->has('cover'))
                        <span class="help-block">
            ...
            <div class="form-group {{ $errors->has('sales_volume') ? 'has-error' : '' }}">
                ...
                <div class="col-sm-10">
                    <input type="number" class="form-control" name="sales_volume" value="{{ $product->sales_volume }}">
